...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template match="permanentLocationId"> <!-- ILN -->
	...
	<xsl:when test=".='4OGW'">5a45e7ca-5eec-4347-9977-d8f30674d568</xsl:when>
        ...
        <xsl:otherwise></xsl:otherwise> <!-- UNDEF -->
      </xsl:choose>
    </permanentLocationId>
  </xsl:template>
...
<!-- MR aus ous205_lnic.asc -->
  ...
  <xsl:template match="permanentLoanTypeId"> <!-- ILN --> <!-- TBD -->
    <permanentLoanTypeId>
      <xsl:choose>
        <xsl:when test=".='0 u Ausleihbar'"><xsl:text></xsl:text></xsl:when>
        <xsl:when test=".='0 c Ausleihbar'"><xsl:text></xsl:text></xsl:when>
        <xsl:when test=".='1 b Kurzausleihe'"><xsl:text></xsl:text></xsl:when>
        <xsl:when test=".='3 s Präsenzbestand'"><xsl:text></xsl:text></xsl:when>
        <xsl:when test=".='3 d Präsenzbestand'"><xsl:text></xsl:text></xsl:when>
        <xsl:when test=".='4 d Passive Fernleihe'"><xsl:text></xsl:text></xsl:when>
        <xsl:when test=".='5 i Nur für den Sonderlesesaal'"><xsl:text></xsl:text></xsl:when>
        <xsl:when test=".='6 f Kurzausleihe 8 Tage'"><xsl:text>???</xsl:text></xsl:when> -->
        <xsl:when test=".='7 o Nur für das Medienzentrum’"><xsl:text></xsl:text></xsl:when>
        <xsl:when test=".='8 e Verlust/vermisst'"><xsl:text></xsl:text></xsl:when>
        <xsl:when test=".='9 a Nicht ausleihbar'"><xsl:text></xsl:text></xsl:when>
        <xsl:when test=".='9 g Nicht ausleihbar'"><xsl:text></xsl:text></xsl:when>
        <xsl:when test=".='9 o Ausleihstatus unbekannt'"><xsl:text></xsl:text></xsl:when>
